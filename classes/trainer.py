...
def train_epoch(model, train_loader, custom_loss, optimizer, epoch):
    ...
    pred_winner_acc /= len(train_loader)
    print(f"Epoch {epoch+1}/{const.EPOCHS}")
    print(f"Train - Loss: {train_loss:.4f}, Winner Acc: {train_winner_acc:.4f}, Odds Acc: {pred_winner_acc:.4f}")
...
def val_epoch(model, val_loader, custom_loss, scheduler, epoch):
    ...
    print(f"Epoch {epoch+1}/{const.EPOCHS}")
    print(f"Val   - Loss: {val_loss:.4f}, Winner Acc: {val_winner_acc:.4f}, Odds Acc: {val_odds_acc:.4f}")
    ...
    if val_loss < best_val_loss:
        ...
        checkpoint_path = os.path.join(const.CHECKPOINT_DIR, f"{const.RUN_NAME}_best.pt")
